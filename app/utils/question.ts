...
export const getCombinedDateMap = (
	questionList: (QuestionBaseSchema & { id: number })[]
): Record<string, QuestionBaseSchemaWithId> => {
	const now = new Date();
    const dateMap: Record<string, QuestionBaseSchemaWithId> = {};
	...
	for (const question of questionList) {
		...
		// 결과 객체에 추가
		dateMap[format(new Date(question.dateAt), 'yyyy-MM-dd')] = {
			...question,
			type,
		};
    }
...
export function formatQuestion(list: (QuestionBaseSchema & {id: number})[]) {
	return list.map(x => {
		let type: QuestionType = 'saved';
